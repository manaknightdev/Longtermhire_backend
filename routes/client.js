  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        let whereClause = "WHERE u.role_id = 'member'";
        let params = [];
        ...
        if (clientName) {
          whereClause += " AND cl.client_name LIKE ?";
          params.push(`%${clientName}%`);
        }
        if (companyName) {
          whereClause += " AND cl.company_name LIKE ?";
        ...
        const countSQL = `
          ...
          ${whereClause}
        `;
        const countResult = await sdk.rawQuery(countSQL, params);
        ...
        const clientsSQL = `
          ...
          FROM longtermhire_user u
          INNER JOIN longtermhire_client cl ON u.id = cl.user_id
          LEFT JOIN longtermhire_client_pricing cp ON cl.user_id = cp.client_user_id
          LEFT JOIN longtermhire_pricing_package pp ON cp.pricing_package_id = pp.id
          ${whereClause}
