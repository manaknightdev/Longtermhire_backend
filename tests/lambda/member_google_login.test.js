...
class GoogleAuthTests {
  ...
  setupTests() {
    this.framework.describe("member Google Authentication Flow", () => {
      // Common test data
      const testData = {
        validCompanyId: "123",
        ...
        role: "member",
        baseState: Buffer.from("longtermhire:_").toString("base64") + "~member",
      };
      ...
      this.framework.addTestCase(
        "member Google Login URL Generation",
        async () => {
          ...
          const response = await this.framework.makeRequest(
            `${BASE_URL}/v2/api/longtermhire/member/lambda/google/login`,
            {
              method: "GET",
            }
          );
      ...
      this.framework.addTestCase(
        "member Google Login with Company ID",
        async () => {
          const response = await this.framework.makeRequest(
            `${BASE_URL}/v2/api/longtermhire/member/lambda/google/login`,
            {
      ...
      this.framework.addTestCase(
        "member Google Code Callback - Success",
        async () => {
          const response = await this.framework.makeRequest(
            `${BASE_URL}/v2/api/longtermhire/member/lambda/google/code`,
            {
              method: "GET",
              query: {
      ...
      this.framework.addTestCase(
        "member Google Code Mobile - Success",
        async () => {
          // Mock successful mobile auth response
          this.framework.mockRequest(
            `${BASE_URL}/v2/api/longtermhire/member/lambda/google/code/mobile`,
            {
              error: false,
      ...
      this.framework.addTestCase("member Google Code - Invalid Code", async () => {
        const response = await this.framework.makeRequest(
          `${BASE_URL}/v2/api/longtermhire/member/lambda/google/code`,
          {
            method: "GET",
      ...
      this.framework.addTestCase("member Rate Limiting Protection", async () => {
